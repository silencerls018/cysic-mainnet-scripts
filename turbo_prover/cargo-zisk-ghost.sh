...
# 3. 等所有 block 的 proof 就绪后，一次性触发 submit_tx.py 上报
...
if [ -n "$CACHE_DIR" ] && [ -d "$CACHE_DIR" ]; then
    ...
    if [ -n "$TASK_HASH" ] && [ -f "$SUBMIT_SCRIPT" ]; then
        # 查找这个 taskHash 对应的所有 block
        ALL_BLOCKS=$(grep -rl "$TASK_HASH" "$TASK_MAP_DIR" 2>/dev/null | xargs -I{} basename {} | sed 's/block_//')
        
        # 收集所有 proof 文件路径
        ALL_PROOF_FILES=""
        ALL_READY=true
        for BLK in $ALL_BLOCKS; do
            PROOF_PATH=$(ls /root/venus_v0_1_6/tmp/prover_7000/${BLK}_*/vadcop_final_proof.bin 2>/dev/null | head -1)
            if [ -z "$PROOF_PATH" ] || [ ! -f "$PROOF_PATH" ]; then
                ALL_READY=false
                log "⏳ Block $BLK 的 proof 还没就绪，暂不上报"
                break
            fi
            if [ -n "$ALL_PROOF_FILES" ]; then
                ALL_PROOF_FILES="${ALL_PROOF_FILES},${PROOF_PATH}"
            else
                ALL_PROOF_FILES="$PROOF_PATH"
            fi
        done

        if [ "$ALL_READY" = true ] && [ -n "$ALL_PROOF_FILES" ]; then
            # 防止重复触发
            SUBMIT_MARKER="/root/cysic-prover/cache/submitted_${TASK_HASH}"
            if [ ! -f "$SUBMIT_MARKER" ]; then
                touch "$SUBMIT_MARKER"
                log "🚀🚀🚀 所有 block proof 就绪！立即交卷！"
                log "  TaskHash: $TASK_HASH"
                log "  ProofFiles: $ALL_PROOF_FILES"
                
                nohup python3 "$SUBMIT_SCRIPT" \
                    --hash "$TASK_HASH" \
                    --proof-files "$ALL_PROOF_FILES" \
                    >> /root/cysic-prover/instant_submit.log 2>&1 &
                
                SUBMIT_PID=$!
                log "🚀 上报进程已启动 PID=$SUBMIT_PID"
            else
                log "⏭️ 任务 $TASK_HASH 已触发过上报，跳过"
            fi
        fi
    else
        if [ -z "$TASK_HASH" ]; then
